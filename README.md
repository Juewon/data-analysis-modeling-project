...
실험 결과, 종속변수인 ECLO에 제곱근 변환을 적용하고 Huber Loss(δ=1.0)를 사용한 모델이 가장 우수한 일반화 성능을 보였습니다. 이에 따라 이후 EDA 과정에서는 제곱근 변환을 반영하여 이상치 영향을 완화한 상태로 분석을 진행했으며, 최종적으로 모델의 기본 손실함수로 Huber Loss(δ=1.0)를 채택하였습니다.
